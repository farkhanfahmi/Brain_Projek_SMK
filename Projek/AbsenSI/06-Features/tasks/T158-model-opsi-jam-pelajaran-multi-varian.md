...
## Acceptance Criteria
- [ ] Admin bisa membuat opsi jam pelajaran baru dengan slot lengkap per hari (termasuk baris istirahat) + cakupan tingkat (Semua/spesifik).
- [ ] Admin bisa mengaktifkan 1 opsi UNTUK 1 TINGKAT TERTENTU tanpa mempengaruhi aktivasi tingkat lain — verified: aktifkan opsi berbeda untuk X vs XII secara bersamaan, keduanya aktif independen.
- [ ] Opsi yang cakupannya TIDAK mencakup tingkat tertentu DITOLAK saat coba diaktifkan untuk tingkat itu, pesan jelas.
- [ ] KALAU Opsi A dipilih: jadwal mengajar existing OTOMATIS mengikuti jam sungguhan dari opsi yang AKTIF UNTUK TINGKAT KELAS itu (verifikasi: ganti opsi aktif tingkat XII saja, cek jam kelas tingkat XII berubah TAPI kelas tingkat X/XI TIDAK berubah).
- [ ] KALAU Opsi B dipilih: jadwal existing TIDAK berubah otomatis (dokumentasikan ini sebagai keputusan final, bukan bug).
- [ ] Tidak ada opsi aktif untuk 1 tingkat = sistem tidak crash untuk tingkat itu, tingkat lain tetap normal.
- [ ] `@LogActivity` terpasang di semua endpoint mutasi (create/update/activate/deactivate/delete).
- [ ] Build + type-check `apps/api` dan `apps/web` hijau.

## Validasi Claudian
- [ ] **WAJIB putuskan Opsi A vs B di poin 3 SEBELUM mulai coding** — ini keputusan arsitektur yang mempengaruhi SELURUH scope task, TIDAK BOLEH diputuskan di tengah jalan.
- [ ] **KALAU Opsi A dipilih**: WAJIB audit SEMUA consumer `Schedule.jamMulai`/`jamSelesai` (untuk `type: jam_mengajar`) sebelum ubah skema — jangan sampai ada halaman/service lain yang diam-diam rusak karena field berubah makna. Pastikan resolusi SELALU tahu tingkat kelas dulu sebelum lookup opsi aktif (bukan asumsi 1 opsi global).
- [ ] Klarifikasi ke user KALAU ragu memilih Opsi A vs B — REKOMENDASI KUAT Opsi A tapi ini keputusan besar yang layak dikonfirmasi eksplisit sebelum kerja besar dimulai.
- [ ] **JANGAN** sentuh `Schedule` untuk `type: jam_sekolah` (T145) — itu domain terpisah total.
- [ ] Konfirmasi constraint "1 aktif" benar-benar PER TINGKAT (`JamPelajaranAktivasi.tingkat @unique`), BUKAN direfaktor diam-diam jadi global lagi karena dianggap "lebih sederhana" — ini requirement eksplisit dari user (revisi 2026-08-13), bukan opsional.
